import type { AiToolOutputType, NormalizedRunAiToolInput } from './ai-tool-core';

type TextOutputType = Exclude<AiToolOutputType, 'image'>;
...
export interface AiToolTextResult {
  title: string;
  summary: string;
  points: string[];
  outputText: string;
...
export interface AiToolTextGenerationResult {
  result: AiToolTextResult | null;
  errorMessage: string;
...
export function fallbackTextResult(content: string, outputType: TextOutputType): AiToolTextResult {
...
export function buildAiToolContent(input: NormalizedRunAiToolInput): string {
  if (!input.fileText) {
    return input.text;
  }
  return [
...
function buildPrompt(content: string, outputType: TextOutputType, hasImage: boolean, hasFile: boolean): string {
  const task = {
...
export async function generateAiToolText(input: NormalizedRunAiToolInput): Promise<AiToolTextGenerationResult> {
