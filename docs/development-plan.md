...
围绕多种 AI 会员建立完整链路：首次登录建档、邀请关系、AI账号注册、套餐购买、后台处理、会员开通计时、T币奖励/抵扣、服务记录展示。
...
- `points_ledger`：记录T币获得、抵扣、返还、回滚，保证幂等和可追溯。
...
- [x] 邀请页：展示邀请人数、T币余额、被邀请人列表；点击邀请好友触发微信分享；`invite-info` 展示T币规则。
- [ ] AI账号注册：会员中心点击 `立即开通` 时先检测是否已注册 AI 账号；未注册先弹注册浮层，保存账号密码后再弹套餐浮层。
- [x] 套餐购买：下单时由后端计算T币抵扣和实付金额。
- [ ] 后台处理：后台处理完成后写入会员记录，状态变为 `active`，会员从处理完成时间开始计时 30 天。
- [ ] 服务记录：展示当前用户订单列表，字段以现有 UI 为准，支持查看服务详情。
- [x] T币奖励与抵扣：好友单笔实付超过 50 元固定奖励 5 T币；累计邀请 10 人额外奖励 10 T币；支付支持使用可用T币抵扣。
...
- 邀请奖励：被邀请用户符合条件的支付成功后，邀请人增加T币并写入 `points_ledger`；重复支付回调不重复奖励。

### 当前需部署云函数

- `user-login`：返回并补齐本地缓存所需的 `openId/openid`，同步微信昵称、头像、邀请码、T币与 AI 账号注册状态。
...
- `get-invite-home`：邀请页读取当前用户邀请码、邀请人数、T币余额、累计获得T币和被邀请人列表。
- `user-login`：新增分享 `inviteCode` 绑定邀请关系逻辑。
- `pay-notify` / `pay-order`：支付完成后复用会员开通逻辑，并为邀请人幂等奖励T币。
