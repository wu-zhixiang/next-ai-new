...
test('compliance display replaces visible fields without changing internal identifiers', () => {
  ...
  assert.equal(compliantProduct.avatarUrl, 'https://example.com/internal.png');
...
test('products and plans without compliance display keep their original fields', () => {
  assert.deepEqual(toCompliantProductType({ ...product, complianceDisplay: undefined }), {
    ...product,
    complianceDisplay: undefined,
  });
  assert.deepEqual(toCompliantPlan({ ...plan, complianceDisplay: undefined }), {
    ...plan,
    complianceDisplay: undefined,
  });
});

test('orders keep original names when compliance metadata is unavailable', () => {
  assert.deepEqual(
