"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.DEFAULT_PRODUCT_NAME = exports.DEFAULT_PRODUCT_CODE = exports.SUCCESS_CODE = exports.COLLECTIONS = void 0;
exports.COLLECTIONS = {
    users: 'users',
    memberPlans: 'member_plans',
    productTypes: 'product_types',
    aiAccountEmailDomains: 'ai_account_email_domains',
    appstoreCountries: 'appstore_countries',
    memberships: 'memberships',
    orders: 'orders',
    inviteRelations: 'invite_relations',
    pointsLedger: 'points_ledger',
    pointsConfig: 'points_config',
    appConfig: 'app_config',
    emailVerificationCodes: 'email_verification_codes',
    appstoreEmailVerificationCodes: 'appstore_email_verification_codes',
    appstoreAccounts: 'appstore_accounts',
    aiNews: 'ai_news',
    aiToolRuns: 'ai_tool_runs',
    aiToolAssets: 'ai_tool_assets',
    aiToolUsageDaily: 'ai_tool_usage_daily',
    aiToolUserUsage: 'ai_tool_user_usage',
    aiToolPointsLedger: 'ai_tool_points_ledger',
    aiToolPointBuckets: 'ai_tool_point_buckets',
    aiToolSingleEntitlements: 'ai_tool_single_entitlements',
    aiToolTemplates: 'ai_tool_templates',
    aiTools: 'ai_tools',
    adminFiles: 'admin_files',
    adminUploadChunks: 'admin_upload_chunks',
    invoiceRequests: 'invoice_requests',
    deliveries: 'deliveries',
    reminderLogs: 'reminder_logs',
    auditLogs: 'audit_logs',
};
exports.SUCCESS_CODE = 0;
exports.DEFAULT_PRODUCT_CODE = 'ai_news';
exports.DEFAULT_PRODUCT_NAME = 'Open AI 资讯会员';
