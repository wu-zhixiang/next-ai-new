...
function isImmediateFulfillmentOrder(order) {
...
async function deductPaymentPointsOnce(order, paidAt) {
    ...
    await (0, db_1.ensureCollection)('pointsLedger');
    ...
    const refreshedUser = await (0, db_1.getUserById)(order.userId);
    const ledger = {
        userId: order.userId,
        orderNo: order.orderNo,
        type: 'payment_deduct',
        direction: 'out',
        points,
        balanceAfter: refreshedUser === null || refreshedUser === void 0 ? void 0 : refreshedUser.pointsBalance,
